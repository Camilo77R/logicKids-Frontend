      ...
      <div className="container">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="welcome-content">
          <div className="welcome-text">
            ...
            <motion.button initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="play-now-btn">
              <Play className="btn-icon" /> Soy Tutor o Padre
            </motion.button>
          </div>
  ...
  <motion.header initial={{ y: -100 }} animate={{ y: 0 }} transition={{ duration: 0.5 }} className="header">
    <div className="header-container">
      ...
      <nav className="header-nav">
        {['Estadísticas', 'Juegos', 'Padres', 'Tutores'].map(item => (
    ...
    <div className="footer-container">
      <div className="footer-grid">
        <div className="footer-brand">
          <div className="footer-logo"><span className="logo-icon"></span><span className="logo-text">LogicKids</span></div>
          <p className="footer-tagline">Making learning fun since 2024</p>
          <div className="social-links">{['📘', '🐦', '📷', '🎵'].map((icon, i) => <a key={i} href="#" className="social-link">{icon}</a>)}</div>
        </div>
        <div className="footer-links">
          {[
            ...
            <div key={i} className="footer-column">
              <h4>{col.title}</h4>
              {col.links.map(link => <a key={link} href={`/${link.toLowerCase().replace(/ /g, '')}`}>{link}</a>)}
            </div>
          ))}
        </div>
      </div>
      <div className="footer-bottom"><p>© 2026 LogicKids. Todos los derechos reservados. Hecho con ❤️ para aprender</p></div>
    </div>
  </footer>
);
